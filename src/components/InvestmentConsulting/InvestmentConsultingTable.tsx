...
import { useInvestmentConsulting, useUpdateInvestmentConsulting, useDeleteInvestmentConsulting } from "@/hooks/api";

import type { InvestmentConsulting } from "@/types/api";

const InvestmentConsultingTable: React.FC = () => {
  const [page, setPage] = useState(1);

  const [search, setSearch] = useState("");

  const { data, isLoading, error } = useInvestmentConsulting({ page, limit: 10 });
  const updateInvestmentConsulting = useUpdateInvestmentConsulting();
  const deleteInvestmentConsulting = useDeleteInvestmentConsulting();

  const handleTogglePublish = async (id: string, currentStatus: boolean) => {
    if (confirm(`آیا از ${currentStatus ? 'عدم انتشار' : 'انتشار'} این صفحه مطمئن هستید؟`)) {
      try {
        await updateInvestmentConsulting.mutateAsync({ id, data: { published: !currentStatus } });
  ...
  const handleDelete = async (id: string) => {
    if (confirm("آیا از حذف این صفحه مطمئن هستید؟ این عملیات قابل بازگشت نیست.")) {
      try {
        await deleteInvestmentConsulting.mutateAsync(id);
    ...
    <div className="rounded-[10px] border border-stroke bg-white shadow-1 dark:border-dark-3 dark:bg-gray-dark">
      <div className="border-b border-stroke px-4 py-4 dark:border-dark-3 sm:px-7.5">
        <div className="flex items-center justify-between">
          <h3 className="font-medium text-dark dark:text-white">لیست صفحات مشاوره سرمایه‌گذاری</h3>
        </div>
      </div>
        ...
        {isLoading ? (
          ...
          <>
            <table className="w-full table-auto">
              ...
              <tbody>
                {data?.items?.map(
                  (item: InvestmentConsulting, index: number) => (
                    <tr key={item.id}>
                      <td
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <Link
                            href={`/investment-consulting/edit/${item.id}`}
                          ...
                          <button
                            ...
                            disabled={updateInvestmentConsulting.isPending}
                          >
                            <svg
                              className="fill-current"
                              width="20"
                          ...
                          <button
                            ...
                            title="حذف"
                            disabled={deleteInvestmentConsulting.isPending}
                          >
                            <svg
                              className="fill-current"
                              width="20"
...
export default InvestmentConsultingTable;
